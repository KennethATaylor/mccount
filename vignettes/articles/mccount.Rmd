...
The estimated MCC at the end of follow-up can be interpreted as the mean number of events we expect in the target population by that time. Looking at the print out from `summary(mcc_overall)`, the MCC is `r round(mcc_final_values(mcc_overall), 2)` - on average, each bladder cancer survivor experiences `r round(mcc_final_values(mcc_overall), 2)` recurrences by `r max(mcc_overall$mcc_table$time)` months.
